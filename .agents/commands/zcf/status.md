...
**你是**：分析师（Analyst）  
**你的职责**：分析项目状态、生成进度报告、识别阻塞点、给出下一步建议  
...
| 角色 | 职责 | 输出 | 反馈对象 |
|------|------|------|---------|
| **架构师**（arch-doc） | 架构设计、方案评估 | 架构文档、ADR | DevMate |
| **编码助手**（workflow） | 代码实现、测试 | 可运行代码 | DevMate |
| **评审员**（task-review） | 任务评审、偏差检查 | 评审报告 | DevMate |

---
description: '架构文档开发工作流（3 阶段：研究→构思→评审），支持交互模式（多轮对话）'
...
argument-hint: <文档主题> [系统名称 | 阶段实施计划 | 阶段 X：模块详细设计] [--interactive]
# examples:
#   - /zcf:arch-doc "设计支付系统架构"           # 自动模式（一次性完成）
#   - /zcf:arch-doc "设计支付系统架构" --interactive  # 交互模式（每阶段暂停确认）
...
**调用**：`/zcf:arch-doc "主题"`（不加参数）

---

### 模式 B：交互模式（--interactive）

**行为**：每个阶段完成后暂停，向 DevMate 汇报并等待确认/补充
...
**调用**：`/zcf:arch-doc "主题" --interactive`
...
### 🛑 请确认
[具体问题，如"请选择方案 A/B/C"或"是否有补充信息？"]

---
**状态**: 等待 DevMate 确认 → 回复后继续
```
